...
export async function POST(request: NextRequest) {
    ...
    if (Array.isArray(inventoryData.currencyInstance)) {
        for (const currencyInstance of inventoryData.currencyInstance) {

            if (currencyInstance.id) {
                await prisma.currencyInstance.update({
                    where: { id: currencyInstance.id },
                    data: {
                        currenyValue: currencyInstance.currenyValue,
                        currenyLabel: currencyInstance.currenyLabel,
                    },
                });
            }
        }
